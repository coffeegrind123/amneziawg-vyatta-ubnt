...
function cfg_private-key {
    ...
    if node_exists private-key; then
        ...
        if [ -f "$PRIVATE_KEY" ]; then
            ...
            sudo awg set $INTERFACE private-key $PRIVATE_KEY
        else
            # Set private-key to value
            echo $PRIVATE_KEY | sudo awg set $INTERFACE private-key /proc/self/fd/0
        fi
    else
        # Remove private-key
...
function cfg_jc {
    if node_exists jc; then
        sudo awg set $INTERFACE jc $(node_value jc)
    fi
}

function cfg_jmin {
    if node_exists jmin; then
        sudo awg set $INTERFACE jmin $(node_value jmin)
    fi
}

function cfg_jmax {
    if node_exists jmax; then
        sudo awg set $INTERFACE jmax $(node_value jmax)
    fi
}

function cfg_s1 {
    if node_exists s1; then
        sudo awg set $INTERFACE s1 $(node_value s1)
    fi
}

function cfg_s2 {
    if node_exists s2; then
        sudo awg set $INTERFACE s2 $(node_value s2)
    fi
}

function cfg_h1 {
    if node_exists h1; then
        sudo awg set $INTERFACE h1 $(node_value h1)
    fi
}

function cfg_h2 {
    if node_exists h2; then
        sudo awg set $INTERFACE h2 $(node_value h2)
    fi
}

function cfg_h3 {
    if node_exists h3; then
        sudo awg set $INTERFACE h3 $(node_value h3)
    fi
}

function cfg_h4 {
    if node_exists h4; then
        sudo awg set $INTERFACE h4 $(node_value h4)
    fi
}

function cfg_i1 {
    if node_exists i1; then
        sudo awg set $INTERFACE i1 $(node_value i1)
    fi
}

function cfg_i2 {
    if node_exists i2; then
        sudo awg set $INTERFACE i2 $(node_value i2)
    fi
}

function cfg_i3 {
    if node_exists i3; then
        sudo awg set $INTERFACE i3 $(node_value i3)
    fi
}

function cfg_i4 {
    if node_exists i4; then
        sudo awg set $INTERFACE i4 $(node_value i4)
    fi
}

function cfg_i5 {
    if node_exists i5; then
        sudo awg set $INTERFACE i5 $(node_value i5)
    fi
}
